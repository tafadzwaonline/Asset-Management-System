...
//********************INVENTORY TITLE*************************************
 ...
 $pdf->Cell(0,85,"Offices Report",0,0, "C");
 //******************END TITLE************************************************

//********************TITLE TWO***********************************************
 $pdf->Ln();
 $pdf->SetFont('Arial','B',16);
 $pdf->Cell(0,-65,"Offices Report For An Inventory Carried Out Between $finalStartDate AND $finalEndDate",0,0, "C");
//*********************END TITLE TWO******************************************
...
// Headers and widths
 ...
 $header = array("OFFICE ID.","OFFICENAME.","OFFICELOCATION.");
...
//fetch db values
 $stmt = $db->prepare("select
       `officestable`.`OfficeId`,
       `officestable`.`OfficeName`,
       `officestable`.`OfficeLocation`
       
  from `officestable` `officestable`
  where  `officestable`.`DateAdded` between
        '$from' and '$to' 
");
 ...
 while($row=$stmt->fetch(PDO::FETCH_ASSOC))
 {
    ...
    $data[] = array($OfficeId,$OfficeName,$OfficeLocation);
