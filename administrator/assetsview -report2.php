...
$originalDate = $from;
$originalEndDate = $to;
$finalStartDate = date("d-m-Y", strtotime($originalDate));
$finalEndDate = date("d-m-Y", strtotime($originalEndDate));
...
//********************INVENTORY TITLE*************************************
 ...
 $pdf->Cell(0,85,"All Assests Report",0,0, "C");
 //******************END TITLE************************************************

//********************TITLE TWO***********************************************
 $pdf->Ln();
 $pdf->SetFont('Arial','B',16);
 $pdf->Cell(0,-65,"Employee Report For An Employee Carried Out Between $finalStartDate AND $finalEndDate",0,0, "C");
//*********************END TITLE TWO******************************************
...
// Headers and widths
 ...
 $header =array("SerialNumber","AssetNumber","AssetName","PurchasePrice","ExpectedAssetLife","ScrapValue","PurchaseDate","Status","AssetCondition");
 $w = array(38,35,35,30,32,35,35,38,38,39);
 $pdf->SetX(10);
...
//fetch db values
 $stmt = $db->prepare("select
       `new_technicianr`.`EmployeeNumber`,
       `new_technicianr`.`EmployeeName`,
       `new_technicianr`.`EmployeePhone`,
       `new_technicianr`.`EmployeeEmail`,
       `new_technicianr`.`EmployeeTown`,
       `new_technicianr`.`Office`,
       `new_technicianr`.`Department`
  from `new_technicianr` `new_technicianr`
 where `new_technicianr`.`DateAdded` between
 '$from' and '$to' 
");
 $stmt->execute();
 while($row=$stmt->fetch(PDO::FETCH_ASSOC))
 {
    ...
    $EmployeeTown = $row['EmployeeTown'];
    $Office = $row['Office'];
    $Department = $row['Department'];
 
 
    $data[] = array($EmployeeNumber, $EmployeeName, $EmployeePhone,$EmployeeEmail,$EmployeeTown,$Office,$Department);
