<?php
	...
	if(!$user->is_logged_in()){ header('Location: login'); }
	    ...
	    $stmt = $db->prepare('SELECT * FROM requested_assets WHERE Id = :assetId');
	    $stmt->execute(array(':assetId' =>$id));
	    $row2 = $stmt->fetch();
	    //if post does not exists redirect user.
	    if($row2['Id'] == ''){
	      header('Location: view-requisitions');
	      exit;
	    }if(($user->checkIfAssigned($row2['SerialNumber'])==$row2['SerialNumber'])){
				header('Location: view-requisitions');
			}
												...
												<div class="step-content pos-rel">
													<div class="step-pane active" data-step="1">
														...
														<?php
															...
															$stmt->execute(array(':assetId' =>$row2['SerialNumber']));
															$row = $stmt->fetch();
														 ?>
														...
														<form class="form-horizontal" id="assignment-form" method="get">
															<div class="form-group">
																...
																<div class="col-xs-12 col-sm-9">
																	<div class="clearfix">
																		<input type="text" name="AssetName" id="AssetName" class="col-xs-12 col-sm-6" value="<?php echo $row['AssetName']; ?>"/>
																	</div>
																</div>
															</div>
															<div class="form-group">
																...
																<div class="col-xs-12 col-sm-9">
																	<div class="clearfix">
																		<input type="text" name="SerialNumber" id="SerialNumber" class="col-xs-12 col-sm-6" value = "<?php echo strtoupper($row['SerialNumber']); ?>"/>
																	</div>
																</div>
															</div>
															<div class="form-group">
																...
																<div class="col-xs-12 col-sm-9">
																	<div class="clearfix">
																		<input type="text" name="AssetNumber" id="AssetNumber" class="col-xs-12 col-sm-6" value = "<?php echo strtoupper($row['AssetNumber']); ?>"/>
																	</div>
																</div>
															</div>
                              <div class="form-group">
																...
																<div class="col-xs-12 col-sm-9">
																	<select id="AssignedUser" name="AssignedUser" class="select2" data-placeholder="Click to Choose..."  onchange="getData(this.value, 'ActivationCode')" >
																		...
																		<<?php
																			...
																			{
																				?>
																				<option value="<?php echo $row_['Id'];?>" <?php if($row2['AssignedUser']==$row_['Id']){echo "selected = 'true'";} ?>> <?php echo $row_['EmployeeName'];?></option>
																				<?php
																			}
																		 ?>
																	</select>
															...
															<div class="form-group" >
																...
																<div class="col-xs-12 col-sm-9">
																	<div class="clearfix">
																		<input type="text" name="UserOffice" id="UserOffice" class="col-xs-12 col-sm-6" disabled value="<?php echo $user->getUserOffice($row2['User']); ?>" />
																	</div>
																</div>
															</div>
															<div class="form-group">
																...
																<div class="col-xs-12 col-sm-9">
																	<div class="clearfix">
																		<input type="text" class="input-sm" id="UserDepartment" name="UserDepartment" disabled value="<?php echo $user->getUserDepartment($row2['User']); ?>"/>
																	</div>
																</div>
															</div>
															<div class="form-group">
																...
																<div class="col-xs-12 col-sm-9">
																	<select id="AssignmentPeriod" name="AssignmentPeriod" class="select2" data-placeholder="Click to Choose..." >
																		<option value="">&nbsp;</option>
																		<option value="1" <?php if($row2['Period']=="1"){echo "selected = 'true'";} ?>>Long term</option>
																		<option value="2" <?php if($row2['Period']=="2"){echo "selected = 'true'";} ?>>Short term</option>
																	</select>
																</div>
															</div>
															<div class="form-group">
																...
																<div class="col-xs-12 col-sm-9">
																	<div class="clearfix">
																		<input type="text" class="input-sm" id="NumberOfDays"  name="NumberOfDays" value="<?php echo $row2['Days']; ?>"/>
																	</div>
																</div>
															</div>
