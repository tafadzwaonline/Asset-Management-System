...
//********************INVENTORY TITLE*************************************
 ...
 $pdf->Cell(0,85,"All Vendors Report",0,0, "C");
 //******************END TITLE************************************************

//********************TITLE TWO***********************************************
 $pdf->Ln();
 $pdf->SetFont('Arial','B',16);
 $pdf->Cell(0,-65,"Vendors Report For An Inventory Carried Out Between $finalStartDate AND $finalEndDate",0,0, "C");
//*********************END TITLE TWO******************************************
...
// Headers and widths
 ...
 $header = array("VendorNumber","VendorName","VendorPhone","VendorEmail", "VendorTown");
...
//fetch db values
 $stmt = $db->prepare("select 
       `new_vendor`.`VendorNumber`,
       `new_vendor`.`VendorName`,
       `new_vendor`.`VendorPhone`,
       `new_vendor`.`VendorEmail`,
       `new_vendor`.`VendorTown`
      
  from `new_vendor` `new_vendor`
 where  `new_vendor`.`DateAdded` between
        '$from' and '$to' 
...
//l cahng
 ...
 while($row=$stmt->fetch(PDO::FETCH_ASSOC))
 {
  ...
  $VendorTown = $row['VendorTown'];
  


  $data[] = array($VendorNumber,$VendorName,$VendorPhone ,$VendorEmail,$VendorTown);
