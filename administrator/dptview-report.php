...
//********************TITLE TWO***********************************************
 ...
 $pdf->Cell(0,-65,"Department Report For An Inventory Carried Out Between $finalStartDate AND $finalEndDate",0,0, "C");
//*********************END TITLE TWO******************************************
...
//fetch db values
 $stmt = $db->prepare("select
       `officedepartments`.`DepartmentId`,
       `officedepartments`.`DepartmentName`,
       `officedepartments`.`ParentOffice`
      
      
       from `officedepartments` `officedepartments`
       where  `officedepartments`.`DateAdded` between
              '$from' and '$to' 
");
 $stmt->execute();
