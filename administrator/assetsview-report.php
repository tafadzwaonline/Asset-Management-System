...
//********************INVENTORY TITLE*************************************
 ...
 $pdf->Cell(0,85,"Asssets Report",0,0, "C");
 //******************END TITLE************************************************

//********************TITLE TWO***********************************************
 $pdf->Ln();
 $pdf->SetFont('Arial','B',16);
 $pdf->Cell(0,-65,"Asset Report For Assets added Between $finalStartDate AND $finalEndDate",0,0, "C");
//*********************END TITLE TWO******************************************
...
// Headers and widths
 ...
 $w = array(38,35,35,30,32,35,35,38,35,35);


 $pdf->SetX(9);
  for($i = 0; $i < count($header); $i++) {
    $pdf->Cell($w[$i], 11, $header[$i], 1, 0, 'C');
  }
 $pdf->Ln();
...
//fetch db values
 $stmt = $db->prepare("select
    ...
    from `new_item` `new_item`
    where  `new_item`.`DateAdded` between
    '$from' and '$to' 
");
 ...
 while($row=$stmt->fetch(PDO::FETCH_ASSOC))
 {
  ...
  // $Vendor= $row['Vendor'];
   ...
   $data[] = array($SerialNumber,$AssetNumber,$AssetName, $PurchasePrice, $ExpectedAssetLife, $ScrapValue,$PurchaseDate,$Status,$AssetCondition);
