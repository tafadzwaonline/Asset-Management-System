										...
										<div class="tabbable">
											<ul id="inbox-tabs" class="inbox-tabs nav nav-tabs padding-16 tab-size-bigger tab-space-1">
												<li class="li-new-mail pull-right">
													<a data-toggle="tab" href="#write" data-target="write" class="btn-new-mail">
														<span>
															<i class="bigger-130"></i>
															<button><span class="bigger-110">Write Message</span></button>
														</span>
													</a>
												</li><!-- /.li-new-mail -->
											...
											<div class="tab-content no-border no-padding">
												<div id="inbox" class="tab-pane in active">
														...
														<div id="id-message-list-navbar" class="message-navbar clearfix">
															<div class="message-bar">
																<div class="message-infobar" id="id-message-infobar">
																	...
																	<span class="grey bigger-110">(<?php echo $user->countUnreadmessages($_SESSION['username']); ?> unread messages)</span>
																</div>
															</div>
										            ...
										            $stmt = $db->prepare('SELECT * FROM private_msg WHERE UserTo = :UserTo ORDER BY Id DESC');
																...
																if($stmt->rowCount() > 0)
										            {
										              ...
										              $stmt2 = $db->prepare('SELECT * FROM private_msg WHERE UserTo = :UserTo ORDER BY Id DESC LIMIT '.$page_position.', '.$item_per_page.'');
										              $stmt2->execute(array(':UserTo' => $_SESSION["Id"]));

										              while($row = $stmt2->fetch())
										              {
																		echo "<div class='";
																		if($row['Opened']=="Yes"){echo "message-item";}else{echo "message-item message-unread";}
																		  echo "'>
																			<label class='inline'>
																			...
																			<i class='message-star ace-icon fa fa-star-o light-";
																			if($row['Opened']=="No"){echo "red";}else{echo "grey";}
																			echo "'></i>
																			<span class='sender' title='".$row['UserFrom']."'>".$row['UserFrom']." </span>
																			<span class='time'>". date('jS M Y', strtotime($row['Date']))."</span>


																			<span class='summary'>
																				<span class='text'>
																				<a href='readmessage?id=".$row['Id']."&page=i'>
																					";if(strlen($row['MessageBody'])> 1500){
																						$msg_body = substr($row['MessageBody'], 0, 5). "...";
																						echo $msg_body;
																					}else {
