<?php
	...
	if(!$user->is_logged_in()){ header('Location: login'); }
	    ...
	    $stmt = $db->prepare('SELECT * FROM assigneditems WHERE Id = :assetId');
	    $stmt->execute(array(':assetId' =>$id));
	    $row_getAssigned = $stmt->fetch();
	    //if post does not exists redirect user.
	    if($row_getAssigned['Id'] == ''){
											...
											<div id="fuelux-wizard-container">
												...
												<div class="step-content pos-rel">
													<div class="step-pane active" data-step="1">
														...
														<?php
														$stmt = $db->prepare('SELECT * FROM new_item WHERE SerialNumber = :SerialNumber OR AssetNumber =:AssetNumber');
														$stmt->execute(array(':SerialNumber' =>$row_getAssigned['SerialNumber'], ':AssetNumber' =>$row_getAssigned['SerialNumber']));
														$row_getSerialAsset = $stmt->fetch();
														?>
														...
														<form class="form-horizontal" id="assignment-form" method="get">
															<div class="form-group">
																...
																<div class="col-xs-12 col-sm-9">
																	<div class="clearfix">
																		<input type="text" name="AssetName" id="AssetName" class="col-xs-12 col-sm-6" value="<?php echo $row_getSerialAsset['AssetName']; ?>"/>
																	</div>
																</div>
															</div>
															...
															<div class="form-group">
																...
																<div class="col-xs-12 col-sm-9">
																	<div class="clearfix">
																		<input type="text" name="SerialNumber" id="SerialNumber" class="col-xs-12 col-sm-6" value = "<?php echo strtoupper($row_getSerialAsset['SerialNumber']); ?>"/>
																	</div>
																</div>
															</div>
															<div class="form-group">
																...
																<div class="col-xs-12 col-sm-9">
																	<div class="clearfix">
																		<input type="text" name="AssetNumber" id="AssetNumber" class="col-xs-12 col-sm-6" value = "<?php echo strtoupper($row_getSerialAsset['AssetNumber']); ?>"/>
																	</div>
																</div>
															</div>
                              <div class="form-group">
																...
																<div class="col-xs-12 col-sm-9">
																	<select id="AssignedUser" name="AssignedUser" class="select2" data-placeholder="Click to Choose..."  onchange="getData(this.value, 'ActivationCode')" >
																		...
																		<<?php
																			...
																			{
																				?>
																				<option value="<?php echo $row['Id'];?>" <?php if($row_getAssigned['User']==$row['Id']){ echo "selected = 'true'";}?>> <?php echo $row['EmployeeName'];?></option>
																				<?php
																			}
																		 ?>
																	</select>
															...
															<div class="form-group" >
																...
																<div class="col-xs-12 col-sm-9">
																	<div class="clearfix">
																		<input type="text" name="UserOffice" id="UserOffice" class="col-xs-12 col-sm-6" disabled value="<?php echo $user->getUserOffice($row_getAssigned['AssignedUser']); ?>" />
																	</div>
																</div>
															</div>
															<div class="form-group">
																...
																<div class="col-xs-12 col-sm-9">
																	<div class="clearfix">
																		<input type="text" class="input-sm" id="UserDepartment" name="UserDepartment"  disabled value="<?php echo $user->getUserDepartment($row_getAssigned['AssignedUser']); ?>"/>
																	</div>
																</div>
															</div>
															<div class="form-group">
																...
																<div class="col-xs-12 col-sm-9">
																	<select id="AssignmentPeriod" name="AssignmentPeriod" class="select2" data-placeholder="Click to Choose..." >
																		<option value="">&nbsp;</option>
																		<option value="1" <?php if($row_getAssigned['AssignmentPeriod']=="1"){echo "selected = 'true'";} ?>>Long term</option>
																		<option value="2" <?php if($row_getAssigned['AssignmentPeriod']=="2"){echo "selected = 'true'";} ?>>Short term</option>
																	</select>
																</div>
															</div>
															<div class="form-group">
																...
																<div class="col-xs-12 col-sm-9">
																	<div class="clearfix">
																		<input type="text" class="input-sm" id="NumberOfDays"  name="NumberOfDays" value="<?php if($row_getAssigned['AssignmentPeriod']=="2"){echo $row_getAssigned['NumberOfDays'];} ?>"/>
																	</div>
																</div>
															</div>
					...
					bootbox.dialog({
						...
						danger: {
							...
							callback: function() {
								...
								$.post('../custom/update-assignAsset.php',
								{
									'SerialNumber': $('[name=SerialNumber]').val(), 'AssignedUser': $('[name=AssignedUser]').val(),
									'NumberOfDays': $('[name=NumberOfDays]').val(), 'AssignmentPeriod': $('[name=AssignmentPeriod]').val(),
									'AssetNumber': $('[name=AssetNumber]').val(), 'Id': "<?php echo $_GET['id'];?>"
