...
//********************INVENTORY TITLE*************************************
 ...
 $pdf->Cell(0,85,"Multiple days Inventory Report",0,0, "C");
 //******************END TITLE************************************************

//********************TITLE TWO***********************************************
 $pdf->Ln();
 $pdf->SetFont('Arial','B',16);
 $pdf->Cell(0,-65,"Inventory Report For An Inventory Carried Out Between $finalStartDate AND $finalEndDate",0,0, "C");
//*********************END TITLE TWO******************************************
...
// Headers and widths
 ...
 $header = array("INVENTORY NO.","ASSET NO.","SERIAL NO.","STATUS", "CONDITION" ,"CUSTODIAN","OFFICE","DEPARTMENT");
...
//fetch db values
 $stmt = $db->prepare("select 
        `new_inventory`.`Id`,
       `new_inventory`.`AssetNumber`,
       `new_inventory`.`AssetSerial`,
       `new_inventory`.`AssetCondition`,
       `new_inventory`.`Status`,
       `new_inventory`.`Custodian`,
       `new_inventory`.`Office`,
       `new_inventory`.`Department`
       from `new_inventory` `new_inventory`
       where `new_inventory`.`DateAdded` between
       '$from' and '$to' 
      ");
 $stmt->execute();
 while($row=$stmt->fetch(PDO::FETCH_ASSOC))
 {
   $InventoryNumber = 'WPC/INVENTORY/'.$row['Id'];
   ...
   $data[] = array($InventoryNumber,$AssetNumber,$SerialNumber,$Status,$Condition,$Custodian,$Office,$Department);
