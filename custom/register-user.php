...
//  if (!is_numeric($user_payroll)) {
//      if (empty($_POST["PayrollNumber"])) {
//     echo = "#dialog-error";//payroll required
//   } else {
//     $user_payroll = test_input($_POST["PayrollNumber"]);
//     // check if name only contains letters and whitespace
//       echo = "#dialog-error";//payroll only contains numbers
//     }else if(strlen($number) != 6) {
//     echo = "#dialog-error"; //The number entered was not 6 digits long";
//     }
//   }
  ...
  if (empty($_POST["email"])) {
    ...
    $user_email = test_input($_POST["UserEmail"]);
...
// check if email or payroll number already enchant_broker_dict_exists
  if(!$user->EmailExists($user_email)){
    if(!$user->PayrollExists($user_payroll)){
      ...
      //insert to db
      $stmt = $db->prepare('INSERT INTO profilemaster(PayrollNumber, Name, Email, Username, PhoneNumber, Password, Photo, Office, Department, OfficeTelephone, Role, Status, tokenCode) VALUES (:PayrollNumber, :Name, :Email, :Username, :PhoneNumber, :Password, :Photo, :Office, :Department, :OfficeTelephone, :Role, :Status, :tokenCode)');
			$stmt->bindParam(':PayrollNumber',$itemName);
			$stmt->bindParam(':Name',$itemPrice);
			$stmt->bindParam(':Email',$itemInitPrice);
			$stmt->bindParam(':Username',$product_brand);
			$stmt->bindParam(':PhoneNumber',$product_category);
			$stmt->bindParam(':Password',$child_category);
			$stmt->bindParam(':Photo',$product_description);
			$stmt->bindParam(':Office',$product_featured);
			$stmt->bindParam(':Department',$imagedata);
			$stmt->bindParam(':OfficeTelephone',$product_new);
			$stmt->bindParam(':Role',$product_recommended);
			$stmt->bindParam(':Status',$product_promoted);
           ...
           $message.='width: 378px;" valign="top" align="left">';
            ...
            $message.='padding-bottom: 10px; padding-top: 15px;" valign="top" align="left">Thank you for signing up!</td></tr></tbody></table>';
             $message.='<div style="font-family: Arial, sans-serif; line-height: 20px; color: #444444; font-size: 13px;">';
               $message.='<b style="color: #777777;">We are excited to have you join the county Asset Manager</b>';
               $message.='<br>';
               $message.='Please click here to complete your registration';
             $message.='</div>';
         ...
         $message.='font-weight: normal; padding-left: 36px; padding-right: 36px;" valign="top" align="left">';
           ...
           $message.='width: 378px;" valign="top" align="left">';
             $message.='<div style="font-family: Arial, sans-serif; line-height: 19px; color: #444444; font-size: 13px; text-align: center;">';
               $message.='<a "http://localhost/ksushopper/change-password?id=".'$id.'&code=".'$code.'" style="color: #ffffff; text-decoration: none; margin: 0px; text-align: center; vertical-align: baseline; border: 4px solid #6fb3e0; padding: 4px 9px; font-size: 15px; line-height: 21px; background-color: #6fb3e0;">&nbsp; Confirm &nbsp;</a>';
             $message.='</div>';
      ...
      echo "#dialog-error"; //The payroll number is already in use;
    }
  }else{
    echo "#dialog-error"; //The email is already in use;
  }
